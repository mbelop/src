...
swcr_authenc(struct cryptop *crp)
{
	...
	for (i = 0; i < crp->crp_ndesc; i++) {
		...
		case CRYPTO_CHACHA20_POLY1305_MAC:
			...
			if (swa->sw_ictx == 0)
				return (EINVAL);
			bcopy(swa->sw_ictx, swa->sw_ctx, axf->ctxsize);
			blksz = axf->blocksize;
			break;
		default:
			return (EINVAL);
	...
	/* Supply MAC with IV */
	if (axf->Reinit)
		axf->Reinit(swa->sw_ctx, iv, ivlen);
	...
	for (i = iskip; i < crda->crd_len; i += axf->hashsize) {
		...
		axf->Update(swa->sw_ctx, blk, axf->hashsize);
	...
	/* Finalize MAC */
	axf->Final(aalg, swa->sw_ctx);
	explicit_bzero(swa->sw_ctx, axf->ctxsize);
