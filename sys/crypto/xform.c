...
{
	return AES_Setkey((AES_CTX *)sched, key, len, 0);
}
...
aes_ctr_setkey(void *sched, u_int8_t *key, int len)
{
	...
	if (AES_Setkey(&ctx->ac_key, key, len - AESCTR_NONCESIZE, 1) != 0)
		return -1;
...
aes_xts_setkey(void *sched, u_int8_t *key, int len)
{
	...
	AES_Setkey(&ctx->key1, key, len / 2, 0);
	AES_Setkey(&ctx->key2, key + (len / 2), len / 2, 0);
